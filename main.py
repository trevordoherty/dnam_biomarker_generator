...
def main():
    ...
    parser.add_argument('Hyperparam', type=str, help='A string indicating whether to optimise hyperparameters using grid search or hyperopt.' + 
                        'methods - pass "RS" or "Hyperopt".')
    ...
    if reducer == False:
        if hyperparam == 'Hyperopt':
            if len(args.Input_File_Path) == 1: 
                best_algo_nestedcv = assess_ML_algorithm_nested_cv_hyperopt(input_data, args.Input_File_Path, args.Results_File_Path, algos)
            elif len(args.Input_File_Path) > 1:
                best_algo_nestedcv = assess_ML_algorithm_nested_cv_hyperopt(input_data, args.Input_File_Path, args.Results_File_Path, algos)
                best_algo_test = assess_ML_algorithm_indep_test_hyperopt(input_data, args.Results_File_Path, algos)
        elif hyperparam == 'RS':
            if len(args.Input_File_Path) == 1: 
                best_algo_nestedcv = assess_ML_algorithm_nested_cv_rs(input_data, args.Input_File_Path, args.Results_File_Path, algos)
            elif len(args.Input_File_Path) > 1:
                best_algo_nestedcv = assess_ML_algorithm_nested_cv_rs(input_data, args.Input_File_Path, args.Results_File_Path, algos)
                best_algo_test = assess_ML_algorithm_indep_test_gs(input_data, args.Input_File_Path, args.Results_File_Path, algos)
    elif reducer == True:
        if len(args.Input_File_Path) == 1: 
            assess_feature_selection_nested_cv_gs(input_data, args.Results_File_Path, algos)
        #elif len(args.Input_File_Path) > 1:
            """
